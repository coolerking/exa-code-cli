 ...
 * EXA_PROXY > HTTPS_PROXY/https_proxy > HTTP_PROXY/http_proxy > config file.
...
function getProxyUrl(proxyOverride?: string): string | undefined {
  ...
  // Check EXA_PROXY first, then standard environment variables
  const exaProxy = process.env.EXA_PROXY;
  const httpsProxy = process.env.HTTPS_PROXY || process.env.https_proxy;
  const httpProxy = process.env.HTTP_PROXY || process.env.http_proxy;
  
  // Priority: EXA_PROXY > HTTPS_PROXY > HTTP_PROXY > config file
  if (exaProxy || httpsProxy || httpProxy) {
    return exaProxy || httpsProxy || httpProxy;
  }
