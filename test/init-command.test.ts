...
function makeTempProject(): string {
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'exa-initcmd-'));
...
test('/init command generates context and posts system message', t => {
  ...
  const messages: any[] = [];
  const original = process.env.EXA_CONTEXT_DIR;
  process.env.EXA_CONTEXT_DIR = root;
  try {
    initCommand.handler({
    ...
    const ctxDir = path.join(root, '.exa');
    ...
    if (original === undefined) {
      delete (process.env as any).EXA_CONTEXT_DIR;
    } else {
      process.env.EXA_CONTEXT_DIR = original;
    }
